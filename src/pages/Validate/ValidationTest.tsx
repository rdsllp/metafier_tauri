...
const ValidationTest = (props: ValidationTestProps) => {
  const [testResult, setTestResult]  = useState<{ [key: string]: any }>(mockTestCriteria);
  ...
  return (
    <div className='w-1/2 h-full flex flex-col'>
      ...
      <Box className="grow min-h-[300px] border-[1px] flex flex-col py-[4px] justify-center rounded
          ...
          <Box key={idx} className="flex w-full text-white">
            ...
            <Box className="px-[8px]">
              <Typography variant="inherit"
                sx={{color: testResult[key].state === 1 ? "#4caf50" : 
                (testResult[key].state === -1 ? "#f44336" : "#ffffff")}}>
                {testResult[key].text}
              </Typography>
            </Box>
          </Box>
